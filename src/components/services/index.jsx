...
export default function ServicesSection() {
  return (
    <div className="flex w-full flex-col gap-6   text-black   items-center justify-center py-6 layout ">
      <h3 className="text-2xl font-semibold underline">Services</h3>
      ...
      <div className="grid w-full items-center justify-center grid-cols-4 gap-4 p-10 bg-secondary rounded-md">
        {Array.from({ length: 8 }).map((_, i) => (
          ...
          >
            ...
            <p key={i} className="text-sm">Home services {i + 1}</p>
          </div>
        ))}
      </div>
    </div>
